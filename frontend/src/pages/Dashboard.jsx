              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex flex-col items-end gap-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-xs font-bold text-gray-400 hover:bg-blue-600 hover:text-white hover:border-blue-500 transition-all shadow-sm"
                  >
                    <Download size={13} />
                    Download Compliance Report
                  </button>
              ...
              <div className="rounded-2xl border border-white/5 bg-[#111827]/70 p-8 shadow-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {debiasStrategies.map((s) => {
                    const isApplied = appliedStrategy === (s.title || s.name || s.id);
                    const isRecommended = s.recommended === true;
                    const diAfter = s.fairness_after?.disparate_impact;
                    const diGain = s.disparate_impact_gain;
                    const accLoss = s.accuracy_loss_pct;
                    return (
                      <div
                        key={s.id}
                        className={`relative group rounded-2xl border p-6 transition-all overflow-hidden ${
                          isApplied
                      ...
                      >
                        ...
                        {isRecommended && !isApplied && (
                          <div className="absolute top-3 right-3 px-2 py-0.5 rounded-md bg-blue-600 text-[9px] font-black text-white uppercase tracking-wider shadow-lg shadow-blue-600/30">
                            ★ Recommended
                          </div>
                        )}
                        ...
                        >
                          {isApplied ? '✓ Applied' : isRecommended ? '★ Apply Recommended' : 'Apply Strategy'}
                        </button>
                      </div>
                    );
                  })}
